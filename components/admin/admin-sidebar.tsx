...
import { LayoutDashboard, FileText, Users, Settings, ChevronLeft, ChevronRight, BarChart3, LogOut } from "lucide-react"
    ...
    >
      ...
      <nav className="flex-1 overflow-y-auto py-4">
        <div className="px-3 py-2">
          <h2
            className={cn(
              "text-xs font-semibold text-gray-400 uppercase tracking-wider transition-opacity",
      ...
      <div className="p-4 border-t border-white/10">
        <Button
          variant="ghost"
          className={cn(
            "w-full text-white hover:bg-white/10 justify-start",
            collapsed ? "justify-center" : "justify-start",
          )}
          onClick={handleLogout}
        >
          <LogOut className={cn("flex-shrink-0", collapsed ? "w-5 h-5" : "w-5 h-5 mr-3")} />
          <span
            className={cn("transition-all duration-300", collapsed ? "opacity-0 w-0 overflow-hidden" : "opacity-100")}
          >
            Sair
          </span>
        </Button>
